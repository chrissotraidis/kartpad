...
plutil -insert CFBundleShortVersionString -string "${KARTPAD_VERSION:-0.4.16}" "${plist}"
plutil -insert CFBundleVersion -string "${KARTPAD_BUILD_NUMBER:-35}" "${plist}"
...
printf '{\n  "SetupVersion": "%s",\n  "SourceCommit": "%s",\n  "UnsignedRuntimeSHA256": "%s"\n}\n' \
  "${KARTPAD_VERSION:-0.4.16}" "${source_commit}" "${unsigned_runtime_hash}" > "${fingerprint}"
