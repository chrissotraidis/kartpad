...
| [#135](https://github.com/chrissotraidis/kartpad/issues/135) iPad opening crash | A10X/iPadOS 17.7.6. Reporter supplied original crash text at 21:28 UTC on 8 September, identifying 0.4.0/build 15, SIGILL and the static-dispatch initializer. Review the supplied instruction/offset and exact old binary correspondence; do not request the same report again or silently match it to the inspected 0.4.11 binary. The generic ARM64 correction is [published in iPhone/iPad build 29](releases/v0.4.13-ios.1.md); physical A10X acceptance remains pending. |
