...
| [#135](https://github.com/chrissotraidis/kartpad/issues/135) iPad opening crash | iPad Pro 10.5/A10X, reported iOS 17.7.6 with jailbreak and several installation methods. Reporter supplied an AI-written SIGILL/dyld/RegisterStaticIndirectDispatchTable summary, not the original crash report; independent binary inspection found an M2 CPU target and LDAPRB in the named initializer, rejected by A10-targeted assembly. Physical-iOS baseline correction and compatible Android preparation were independently reviewed; manual Apple PR146 merged the correction and owner reports a fresh build/ISA audit passing; final merged-source artifact handoff pending. Matching original crash PC/UUID is still needed before attributing the reporter failure. Public 0.4.11 declares arm64 and minimum iOS 16, not proof of A10X acceptance. Asked whether any chooser appears, exact IPA and matching system crash Exception/Termination/crashed-thread excerpt; no reinstall or in-app export request. No established jailbreak/GPU cause or new-build fix. |
