...
Open-issue audit: 20 issues checked at 21:52 UTC on 8 September 2026.
...
| #135 iPad launch crash | CPU correction published; original crash text now supplied | Review the 8 September 21:28 UTC report for 0.4.0/build15 against that binary; then obtain exact build29 A10X acceptance. Do not repeat the raw-report request | A10X/iPadOS; this does not validate Android CPU or graphics behavior |
| #105 rating companion restore | Published and reporter-confirmed for offline ratings/information transfer | Acknowledge the 8 September 14:06 UTC success; preserve separate Mii/server-sync scope | Android; Apple parity not implemented |
...
| #103 and owner Pixel sustained FPS | Historical physical profiles identify FP status/context and resource-hashing costs; current-build attribution needed | Follow [targeted handoff](ANDROID-PERFORMANCE-HANDOFF.md). Reprofile warmed actual gameplay; benchmark reviewed scalar-context candidate a396eda independently before expanding scope. Do not equate multiply microbenchmark gains with FPS | Android; standard power mode and comparable thermals, separate from #123 |
...
| #142 disclosure/docs | Answered; disclosure and README/docs cleanup merged through PR #148 | Keep disclosure and documentation consistent; leave broader UI review separate from runtime investigation | Repository/support; issue remains open |
